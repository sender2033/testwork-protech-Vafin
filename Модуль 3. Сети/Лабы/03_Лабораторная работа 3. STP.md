...
Это все говно, оно не работает
Что нужно сделать завтра
вообще схема транка 
речь про питерские 
sw1_acs - sw2_acs vlan 
...
PiterSW2_ACS(config-if)#switchport trunk allowed vlan 71,77,112,999
PiterSW2_ACS(config-if)#exit
```
PiterSW1_ACS
```
...
PiterSW1_ACS(config-if)#switchport trunk allowed vlan 71,77,112,999
PiterSW1_ACS(config-if)#switchport trunk native vlan 999
PiterSW1_ACS(config-if)#exit
```
PiterSW1_Core
```
PiterSW1_Core(config)#interface range e0/0-3
...
PiterSW1_Core(config-if-range)# switchport trunk allowed vlan 77,112
PiterSW1_Core(config-if-range)# no shutdown
