...
PiterSW2_ACS(config-if)#switchport trunk allowed vlan all
...
PiterSW1_ACS(config-if)#switchport trunk allowed vlan all
PiterSW1_ACS(config-if)#switchport nonegotiate
PiterSW1_ACS(config-if)#exit
PiterSW1_ACS(config)#interface e0/1
PiterSW1_ACS(config-if)#switchport trunk encapsulation dot1q
PiterSW1_ACS(config-if)#switchport mode trunk
PiterSW1_ACS(config-if)#switchport trunk allowed vlan all
PiterSW1_ACS(config-if)#switchport nonegotiate
...
switchport trunk encapsulation dot1q
 ...
 switchport trunk allowed vlan 71,77,112,999