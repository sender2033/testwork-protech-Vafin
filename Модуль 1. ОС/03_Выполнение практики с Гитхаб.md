...
5. Для выключения usb-контроллера нужно их сначала найти `lspci | grep -i usb && lsusb -t`/. Далее определяем модуль ядра `lspci -k -s 02:00.0 && lspci -k -s 02:03.0`.  Следующий шаг - отключение модулей `sudo modprobe -r uhci_hcd && sudo modprobe -r xhci_hcd`
