...
2. Назначаем права доступа на файл 007 `chmod u=,g=,o=rwx file2.txt` , таким образом user1 полные права к этому файлу, т.к. он попадает под категорию "Другие". Для проверки переключаюсь на user1 и пр