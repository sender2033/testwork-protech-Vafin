...
4. Возвращаем все как было: удаляем файл `rm /etc/modprobe.d/blacklist-rapl.conf` и загружаем модуль `modprobe intel_rapl_msr`. Проверяем изменения командой `lsmod | grep intel_rapl_msr` ![Image]()